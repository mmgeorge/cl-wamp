(defpackage :wamp/session
  ...
  (:import-from :wamp/transport/rawsocket #:make-rawsocket)
  ...
  (:export #:session #:make-session #:id #:transport
           #:start #:register #:unregister #:call
           #:timeout-exceeded
           #:registration-id #:registration-uri #:registration-procedure
           ))
...
(defclass session ()
  ((transport
   ...
   (url
    :documentation "Url for the target websocket server"
    :reader url :initarg :url :type string)
   (port
    :reader port :initarg :port :type number)
   (realm
...
#[(dtype (string number string &key (:log-output stream) (:log-serious-conditions boolean) (:log-verbose boolean)) session)]
(defun make-session (url port realm
  ...
  (make-instance 'session :url url :port port :realm realm
...
(defun -handle-close (self &key code reason)
  ...
  (when (and code (not (eq code 1000))) ;; Can't use = here because code can be NIL
    (-log self "Socket closed unexpectedly (~a) Attempting to reconnect" code)
    (setf (id self) 0)
...
(defun -connect-transport (self)
  (let* ((on-message (lambda (type args) (-handle-message self type args)))
         ;;(on-error (lambda (error) (-log self "Transport encountered error ~a" error)))
         (on-close (lambda (&key code reason) (-handle-close self :code code :reason reason)))
         (transport (make-rawsocket (url self) (port self)
...
;;#[(dtype (session string function &key
  ;;        (:match match-t) (:invoke string) (:disclose_me boolean)) (promise-of registration))]
(defun register (self uri procedure &key match invoke disclose_me)
  ...
  (let* ((options (-make-options (pairlis '(match invoke disclose_me) (list match invoke disclose_me))))
...
(defun call (self uri &key args kwargs)
  (let ((options (-make-options `((disclose_me . t ))))
...
(defun -send-await (self type args)
  ...
  (format t "Debug: Sending message: type: ~a args: ~a" type args)
